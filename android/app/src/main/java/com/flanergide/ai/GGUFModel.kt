        ...
        if (!outputFile.exists()) {
            ...
            try {
                ...
                try {
                    ...
                    // File is not in assets (expected in development without model file)
                    Log.w(tag, "⚠️ Model file not found in assets: $assetPath")
                    Log.w(tag, "⚠️ This is normal in development. The app will use mock responses.")
                    Log.i(tag, "💡 To use the real model, add app/src/main/assets/models/phi-2-q4_k_m.gguf (1.6 GB)")
                    throw IllegalStateException("Model file not in assets. Using mock generation instead.")
                }
