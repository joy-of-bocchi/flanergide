...
object TextCaptureEngine {
    ...
    /**
     * Print all captured text to logcat (for debugging).
     */
    fun dumpLog() {
        synchronized(capturedTextLog) {
            Log.i(TAG, "=== Captured Text Log (${capturedTextLog.size} entries) ===")
            capturedTextLog.forEachIndexed { index, captured ->
                Log.i(TAG, "[$index] ${captured.appPackage}: ${captured.text.take(80)}")
            }
            Log.i(TAG, "=== End of Log ===")
        }
    }
}
